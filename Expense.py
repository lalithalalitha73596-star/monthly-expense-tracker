...
if not st.session_state.logged_in:
    ...
    if menu == "📊 Dashboard Overview":
        if total_expense > st.session_state.monthly_budget:
            st.error(f"🚨 ALERT: You have exceeded your monthly budget! You spent ₹ {total_expense:,.2f}, which is over your limit of ₹ {st.session_state.monthly_budget:,.2f}.")
        elif total_expense >= (st.session_state.monthly_budget * 0.9):
            st.warning(f"⚠️ Budget Warning: You have used over 90% of your monthly budget (₹ {total_expense:,.2f} / ₹ {st.session_state.monthly_budget:,.2f}).")
        # --- BUDGET ALERT LOGIC END ---
        ...
        with b_col1:
            ...
            with get_db_connection() as conn:
                cursor = conn.cursor()
                cursor.execute(query, (st.session_state.user_id, st.session_state.user_id))
                rows = cursor.fetchall()

            df_rec = pd.DataFrame(rows, columns=["Date", "Type", "Category", "Amount", "Details"])
            
            # Dynamic header display based on active dataset rows length
            entry_count = len(df_rec)
            st.markdown(f"<div class='content-card'><h3>⏱️ Last {entry_count} Entry(s) Added</h3>", unsafe_allow_html=True)
        ...
        with b_col2:
            ...
            '''
            with get_db_connection() as conn:
                df_export = pd.read_sql_query(export_query, conn, params=(st.session_state.user_id, st.session_state.user_id))
