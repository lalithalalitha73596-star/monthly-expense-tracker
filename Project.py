    ...
    if menu == "📊 Dashboard Overview":
        ...
        with b_col1:
            ...
            df_rec = pd.read_sql_query(
                "SELECT expense_date as Date, category as Category, amount as [Amount (₹)], note as Details FROM expenses WHERE user_id=? ORDER BY expense_id DESC LIMIT 3", 
        ...
        with b_col2:
            ...
            df_export = pd.read_sql_query("SELECT expense_date, category, amount, note FROM expenses WHERE user_id=?", conn, params=(st.session_state.user_id,))
